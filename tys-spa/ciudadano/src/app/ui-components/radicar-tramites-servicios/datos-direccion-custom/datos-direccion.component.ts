...
@Component({
  selector: 'app-datos-direccion-custom',
  templateUrl: './datos-direccion.component.html',
})
export class DatosDireccionCustomComponent extends FormBaseComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.paisOptions$ = this.paisProvider.list()
      .map((paises) => {
  ...
  save() {
    ...
    if (this.form.valid) {
      ...
      this.onSave.emit({index: this.indexOrg, contacts: this.contacts});
    }
  }
